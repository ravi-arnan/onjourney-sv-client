...
- Dokumentasi: [`docs/pipeline-staging-main.md`](pipeline-staging-main.md) (peran branch, alur harian, promosi, hotfix, diagram Mermaid, setup awal, saran pengaturan GitHub).
- [`README.md`](../README.md) bagian Git diarahkan ke alur **staging** sebagai basis cabang dan PR; **`main`** hanya lewat promosi atau hotfix; checklist promosi ditambahkan.
- [`.github/workflows/ci.yml`](../.github/workflows/ci.yml): CI pada push/PR ke `staging` dan `main` (`bun install --frozen-lockfile`, `check`, `lint`, `test:unit`).
- [`.github/dependabot.yml`](../.github/dependabot.yml): `target-branch: staging` agar bump dependency masuk integrasi dulu.

### Decisions

- Pemisahan integrasi (`staging`) dan rilis (`main`) menggantikan model “PR langsung ke `main`” di dokumentasi sebelumnya; hotfix dari `main` dengan merge balik ke `staging`.
- CI memakai unit test saja untuk `test` (bukan e2e Playwright) agar cepat dan stabil di runner; e2e tetap bisa dijalankan lokal/`bun run test`.

### Next Steps

- Buat branch `staging` di remote dan set default branch jika belum; pastikan branch protection memakai status CI.
- Jika `bun run lint` gagal di CI karena utang format Prettier, jalankan `bun run format` pada branch terpisah atau longgarkan gate sampai baseline bersih.
