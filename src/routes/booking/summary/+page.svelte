	...
	<Container>
		<div class="left">
			...
			<div class="info-banner">
				...
				<p>
					Payment instructions also have been sent to each email. Once your payment is confirmed, we
					will send your trip e-ticket to your email address.
				</p>
			</div>
				...
				<div class="va-card">
					...
					<div class="va-row">
						...
						<div class="va-value">
							...
							<button
								class="copy-btn"
								class:copied={copied === 'va'}
								on:click={() => copy(vaInfo.number, 'va')}
							>
					...
					<div class="va-row">
						...
						<div class="va-value1">
							...
							<button
								class="copy-btn"
								class:copied={copied === 'amt'}
								on:click={() => copy(total, 'amt')}
							>
					...
					<button class="show-instruction-btn" on:click={() => toggleInstruction('full')}>
						...
						<svg
							...
							stroke="currentColor"
							stroke-width="2"
							style="transform: rotate({showInstruction['full']
								? 180
			...
			{#if paymentType === 'full' && method === 'ewallet'}
				...
				<div class="va-card ewallet-card">
					...
					<p class="ewallet-info">
						Open your <strong>{walletLabel[wallet] ?? wallet}</strong> app and complete the payment. You
      ...
      ══════════════════════════════════════════ -->
							...
							{#if split.method === 'bank_transfer'}
								...
								<div class="va-row">
									...
									<div class="va-value">
										...
										<button
											class="copy-btn"
											class:copied={copied === vaKey}
											on:click={() => copy(va.number, vaKey)}
										>
								...
								<div class="va-row">
									...
									<div class="va-value1">
										...
										<button
											class="copy-btn"
											class:copied={copied === amtKey}
											on:click={() => copy(split.amount, amtKey)}
										>
											{copied === amtKey ? 'Copied!' : 'Copy'}
										</button>
									</div>
								</div>
								...
								<button class="show-instruction-btn" on:click={() => toggleInstruction(instrKey)}>
									...
									<svg
										...
										stroke="currentColor"
										stroke-width="2"
										style="transform: rotate({showInstruction[instrKey]
											? 180
											: 0}deg); transition: transform 0.2s"
									>
										<polyline points="6 9 12 15 18 9" />
								...
								{#if showInstruction[instrKey]}
									<ol class="instruction-list">
										<li>
											Login to your {(split.bank ?? 'BCA').toUpperCase()} mobile banking or ATM
										</li>
										<li>Select <strong>Transfer → Virtual Account</strong></li>
										<li>Enter the virtual account number above</li>
										<li>Confirm the transfer amount matches exactly</li>
		...
		<div class="right">
			<div class="order-card">
				...
				<a href={resolve('/booking/confirmation')} style="display:block">
					<button class="cta-btn">See Booking Status</button>
				</a>
				<p class="terms-note">
					By clicking the Proceed Payment, you agree to
					<a href={resolve('/terms')} class="terms-link">On Journey's Terms & Conditions</a> &
					<a href={resolve('/privacy')} class="terms-link">Privacy Policy</a>
				</p>
			</div>
		</div>
	</Container>
